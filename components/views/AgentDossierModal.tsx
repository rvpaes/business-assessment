      ...
      <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200">
        ...
        <div className="p-4 sm:p-6 border-b border-slate-100 flex items-center justify-between sticky top-0 bg-white z-10">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl ${data.avatarBg} text-white flex items-center justify-center font-black text-sm shadow-xs shrink-0`}>
              {data.avatarLetter}
            </div>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="text-sm sm:text-base font-black text-slate-900">{data.agentName}</h3>
                <span className="px-2 py-0.5 rounded text-[10px] font-extrabold bg-[#074878] text-white">
                  {data.badge}
                </span>
              </div>
              <p className="text-[11px] sm:text-xs text-slate-500">{data.agentRole} • BigQuery SLA: {data.latencyMs || 140}ms</p>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-6 space-y-6 text-xs">
          ...
          {data.sellerPlaybook && (
            <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-blue-950 to-[#074878] text-white space-y-3.5 border border-blue-800/60 shadow-md">
        ...
        <div className="p-4 border-t border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-slate-50/50 rounded-b-3xl">
          <span className="text-[11px] text-slate-500 flex items-center justify-center sm:justify-start gap-1">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
            100% Auditável no BigQuery Property Graph
          </span>
          <button
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl bg-[#074878] hover:bg-[#053456] text-white font-bold text-xs shadow-xs transition-colors cursor-pointer w-full sm:w-auto text-center"
          >
            Fechar Dossiê
          </button>
        </div>
